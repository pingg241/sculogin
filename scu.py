...
class SCULoginSystem:
    ...
    def get_captcha(self):
        ...
        if resp.status_code == 200:
            ...
            captcha_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f'captcha_{ts}_{rand}.jpg')
    ...
    def login(self, username, password, max_retry=3):
        for attempt in range(1, max_retry + 1):
            ...
            try:
                ...
                continue
            try:
                for f in glob.glob(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'captcha_*.jpg')):
                    try:
                        os.remove(f)
                    except Exception:
                        pass
...
def cookies():
    ...
    if os.path.exists('cookies.json'):
        ...
        try:
            ...
            for _ in range(5):
                res = login_system.session.get(url, headers=login_system.headers, timeout=5, allow_redirects=False)
                if res.status_code == 302:
                    location = res.headers.get('Location', '')
                    if not location:
                        print("302跳转但未获取到Location")
                        login_system = None
                        return False
                    url = login_system.base_url + location if location.startswith('/') else location
                    continue
                elif res.status_code == 200:
                    match = re.search(r'<strong>发生错误！</strong>([^<]*)', res.text)
                    if match:
                        print("发生错误：", match.group(1).strip())
                        login_system = None
                        return False
                    if "登录" not in res.text and "j_spring_security_check" not in res.text:
                        print("登录状态未过期，无需登录")
                        return True
                    else:
                        print("cookies已过期")
                    ...
                    print(f"未知响应码：{res.status_code}")
                    print(res.text)
                    login_system = None
                    return False
            print("跳转次数过多，疑似cookies已过期")
            login_system = None
            return False
        except Exception as e:
            print("刷新异常：", e)
